...
public class RouteMapGenerator implements IGui {
	...
	private static void drawDiagonalLine(NativeImage nativeImage, int x1, int y1, int x2, int y2, int color) {
    	final int dx = Integer.signum(x2 - x1);
    	final int dy = Integer.signum(y2 - y1);
    	final int steps = Math.abs(x2 - x1);
    	final int halfLineHeight = lineSize / 2;
    	...
    	for (int i = 0; i <= steps; i++) {
        	...
        	for (int ox = -halfLineHeight; ox <= halfLineHeight; ox++) {
            	for (int oy = -halfLineHeight; oy <= halfLineHeight; oy++) {
                	if (Math.abs(ox) + Math.abs(oy) <= halfLineHeight * 1.5) {
	...
	private static void drawLine(NativeImage nativeImage, StationPosition stationPosition1, StationPosition stationPosition2, float widthScale, float heightScale, float xOffset, float yOffset, int color) {
    	...
    	if (Math.abs(x2 - x1) == Math.abs(y2 - y1)) {
        	drawDiagonalLine(nativeImage, x1, y1, x2, y2, color);
    	} else if (Math.abs(x2 - x1) > Math.abs(y2 - y1)) {
        	drawStraightLine(nativeImage, x1, y1, x2, y1, color);
        	drawStraightLine(nativeImage, x2, y1, x2, y2, color);
    	} else {
        	drawStraightLine(nativeImage, x1, y1, x1, y2, color);
        	drawStraightLine(nativeImage, x1, y2, x2, y2, color);
    	}
	}

	private static void drawStraightLine(NativeImage nativeImage, int x1, int y1, int x2, int y2, int color) {
    	final int halfLineHeight = lineSize / 2;
    	final boolean isHorizontal = y1 == y2;
    
    	if (isHorizontal) {
        	final int minX = Math.min(x1, x2);
        	final int maxX = Math.max(x1, x2);
        	for (int x = minX; x <= maxX; x++) {
            	for (int yOffset = -halfLineHeight; yOffset <= halfLineHeight; yOffset++) {
                	drawPixelSafe(nativeImage, x, y1 + yOffset, color);
            	}
        	}
    	} else {
        	...
        	for (int y = minY; y <= maxY; y++) {
            	for (int xOffset = -halfLineHeight; xOffset <= halfLineHeight; xOffset++) {
                	drawPixelSafe(nativeImage, x1 + xOffset, y, color);
            	}
        	}
    	}
	}
