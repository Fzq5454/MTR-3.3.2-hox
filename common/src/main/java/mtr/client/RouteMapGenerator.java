...
public class RouteMapGenerator implements IGui {
    ...
    public static NativeImage generateRouteMap(long platformId, boolean vertical, boolean flip, float aspectRatio, boolean transparentWhite) {
        ...
        try {
            ...
            if (routeCount > 0) {
                ...
                for (int routeIndex = 0; routeIndex < routeCount; routeIndex++) {
                    ...
                    for (int stationIndex = 0; stationIndex < platformIds.size(); stationIndex++) {
                        if (stationIndex != currentIndex) {
                            ...
                            if (stationIndex < currentIndex) {
                                stationsIdsBefore.get(routeIndex).add(0, stationId);
                            } else {
                                stationsIdsAfter.get(routeIndex).add(stationId);
                            }
                        }
                    }
                ...
                final Route mainRoute = routeDetails.get(0).getA();
                ...
                for (int routeIndex = 0; routeIndex < routeCount; routeIndex++) {
                    ...
                    final boolean isBranchLine = route != mainRoute;
                    ...
                    for (int stationIndex = 0; stationIndex < route.platformIds.size(); stationIndex++) {
                        ...
                        if (stationIndex < route.platformIds.size() - 1) {
                            final StationPosition nextPosition = routeStationPositions.get(stationIndex + 1 - currentIndex);
                            drawLine(nativeImage, stationPosition, nextPosition, widthScale, heightScale, 
                                     xOffset, yOffset, stationIndex < currentIndex ? ARGB_LIGHT_GRAY : ARGB_BLACK | route.color,
                                     isBranchLine);
                        }
    ...
    private static void drawLine(NativeImage nativeImage, StationPosition stationPosition1, StationPosition stationPosition2, float widthScale, float heightScale, float xOffset, float yOffset, int color, boolean isBranchLine) {
        ...
        if (!isBranchLine) {
            ...
            final int steps = Math.max(xChangeAbs, yChangeAbs);
            final float dx = (float) xChange / steps;
            final float dy = (float) yChange / steps;
            
            for (int i = 0; i <= steps; i++) {
                final int px = Math.round(x1 + dx * i);
                final int py = Math.round(y1 + dy * i);
                drawLineSegment(nativeImage, px, py, lineSize, color);
            }
        }
    }

    private static void drawLineSegment(NativeImage nativeImage, int x, int y, int size, int color) {
        final int halfSize = size / 2;
        for (int ox = -halfSize; ox <= halfSize; ox++) {
            for (int oy = -halfSize; oy <= halfSize; oy++) {
                if (Math.sqrt(ox * ox + oy * oy) <= halfSize) {
                    drawPixelSafe(nativeImage, x + ox, y + oy, color);
                }
            }
        }
    }
